...
# ── 人员档案 ──────────────────────────────────────────────────
PEOPLE = {
    "alice":   {"dept": "算法工程", "role": "算法工程师",    "expertise": ["强化学习", "AI 推荐决策"]},
    "bob":     {"dept": "产品运营", "role": "产品经理",      "expertise": ["视频产品运营", "用户体验"]},
    "carol":   {"dept": "算法工程", "role": "算法实习生",    "expertise": ["机器学习", "Python"]},
    "david":   {"dept": "基础设施", "role": "基础设施工程师", "expertise": ["系统设计", "数据库"]},
    "eve":     {"dept": "数据洞察", "role": "数据洞察师",    "expertise": ["数据洞察", "SQL", "BI"]},
    "frank":   {"dept": "产品运营", "role": "产品总监",      "expertise": ["产品战略", "商业化"]},
    "grace":   {"dept": "算法工程", "role": "算法负责人",    "expertise": ["推荐系统", "RL", "决策"]},
}
...
MEETINGS = [
    # M1: 灰度决策会（算法工程 × 产品运营）
    {
        ...
        "topic": "AI 推荐决策模型灰度发布",
        "transcript": """
[alice] 本次会议确认 AI 推荐决策模型（20260309版本）的灰度方案。
[bob]   产品运营侧同意先走 1% 灰度，观察 48 小时。
[alice] 好，灰度比例定 1%，观察期 48 小时，熔断阈值是BufferRate上涨超 5%。
[bob]   alice 负责今天完成部署，我明天发监控日报，48h 后我们复盘。
[alice] 还有一个点，content_score_v1 字段本周内我出接口文档。
        """.strip(),
        "expected_extract": {
            "decisions": [
                "AI 推荐决策模型灰度比例 1%",
                "观察期 48 小时，熔断阈值BufferRate上涨超 5%",
            ],
            "action_items": [
                {"owner": "alice", "task": "完成灰度部署"},
                {"owner": "bob",   "task": "发监控日报"},
                {"owner": "alice", "task": "出 content_score_v1 接口文档"},
            ],
        },
    },
    # M2: 扩量决策（48h 复盘）
    {
        ...
        "topic": "AI 推荐决策灰度扩量",
        "transcript": """
[alice] 灰度 48h 观察通过，BufferRate无异常，DropRate下降 12%。
[frank] 数据好看，我同意扩量，建议直接到 5%。
[bob]   产品运营侧没有问题，5% 可以接受。
[alice] 好，灰度从 1% 扩到 5%，生效时间 2026-04-17。
[bob]   后续每日发日报，alice 我这边需要你的监控 dashboard 权限。
        """.strip(),
        "expected_extract": {
            "decisions": [
                "灰度从 1% 扩量到 5%",
            ...
            "action_items": [
                {"owner": "alice", "task": "授权监控 dashboard 给 bob"},
            ],
        },
    },
    # M3: 数据洞察评审（算法工程 × 数据洞察）
    {
        ...
        "topic": "离线评估指标对齐",
        "transcript": """
[eve]   我们数据洞察侧对 QualityScore 的口径和你们不一样，需要对齐。
[alice] 我们用的是 frame_metrics 修正后的 QualityScore，你们呢？
[eve]   我们用的是原始 QualityScore，没有修正。
[alice] 那差异会比较大，高画质场景下差距可能有 0.3%。
[eve]   好，我们统一用修正后的 QualityScore，我来更新 BI 报表。
[alice] 另外DropRate的分母定义也要对齐，我发你文档。
        """.strip(),
        "expected_extract": {
            "decisions": [
                "统一使用 frame_metrics 修正后的 QualityScore",
                "DropRate分母定义需对齐",
            ],
            "action_items": [
                {"owner": "eve",   "task": "更新 BI 报表口径"},
                {"owner": "alice", "task": "发DropRate分母定义文档给 eve"},
            ],
        },
    },
    # M4: 基础设施对接（算法工程 × 基础设施）
    {
        ...
        "topic": "SDK 推理性能优化",
        "transcript": """
[david] 我看了你们的 SDK，初始化耗时 41ms，单次推理 4.6µs，有优化空间吗？
[alice] 初始化主要是加载三个模型（DNN + BufferMetrics + DropMetrics），可以做懒加载。
[david] 懒加载我来做，你这边需要提供模型文件的 path 配置接口。
[alice] 好，我这周提供配置接口，下周联调。
[david] 另外并发场景下有没有锁？我们压测发现了偶发的竞争问题。
[alice] 有，读写锁在 predict 接口上，但 init 阶段没有，我去修一下。
        """.strip(),
        "expected_extract": {
            "decisions": [
                "SDK 初始化改为懒加载",
                "alice 本周提供模型 path 配置接口",
            ],
            "action_items": [
                {"owner": "david", "task": "实现 SDK 懒加载"},
                {"owner": "alice", "task": "提供模型 path 配置接口"},
                {"owner": "alice", "task": "修复 init 阶段缺少锁的问题"},
            ],
        },
    },
    # M5: 算法组内部周会（算法工程内部）
    {
        ...
        "topic": "模型蒸馏实验进展",
        "transcript": """
[grace] carol 先讲一下 决策树模型蒸馏的进展。
[carol] depth=13 时验证集准确率 85.58%，与教师模型一致性 88.43%。
[grace] 这个一致性还行，但有个 bug 要修：model_distill.py 第123行 wide_score 系数写死了。
[alice] 对，应该用 config 里的系数，我来修，修完需要重跑实验。
[grace] 好，carol 负责重跑对比实验，alice 这周修好代码。
        """.strip(),
        "expected_extract": {
            "decisions": [
                "决策树模型 depth=13，一致性 88.43%",
                "修复 model_distill.py 第123行系数 bug",
            ],
            "action_items": [
                {"owner": "alice", "task": "修复 model_distill.py wide_score 系数 bug"},
                {"owner": "carol", "task": "重跑 ModelDistill 对比实验"},
            ],
        },
    },
    # M6: 全量发布评审（多部门）
    {
        ...
        "topic": "AI 推荐决策全量发布评审",
        "transcript": """
[grace] 灰度在 5% 跑了两周，各项指标稳定，今天评审全量发布。
[frank] 产品侧没问题，我已经和业务确认过了。
[bob]   产品运营侧同意，但需要保留快速回滚能力，回滚时间要在 5 分钟内。
[alice] 回滚方案已经准备好，通过配置开关切换，5 分钟内可以完成。
[grace] 好，批准全量发布，发布时间定 2026-05-01，grace 负责发布公告。
[alice] 我负责发布操作，frank 你这边需要更新产品文档。
        """.strip(),
        "expected_extract": {
            "decisions": [
                "批准 AI 推荐决策 全量发布",
            ...
            "action_items": [
                {"owner": "grace", "task": "发布全量发布公告"},
                {"owner": "alice", "task": "执行全量发布操作"},
                {"owner": "frank", "task": "更新产品文档"},
            ],
        },
    },
]
...
SEARCH_QA = [
    ...
    {"id": "Q2",  "category": "跨人检索", "query": "扩量",        "caller": "carol", "caller_dept": "算法工程",
     "expected_owners": ["alice"], "expected_tags": ["扩量", "灰度"], "min_results": 1},
    {"id": "Q3",  "category": "跨人检索", "query": "熔断",        "caller": "david", "caller_dept": "基础设施",
     "expected_owners": ["alice"], "expected_tags": [], "min_results": 1},
    {"id": "Q4",  "category": "跨人检索", "query": "QualityScore",       "caller": "david", "caller_dept": "基础设施",
     "expected_owners": ["alice", "eve"], "expected_tags": [], "min_results": 1},
    {"id": "Q5",  "category": "跨人检索", "query": "ModelDistill",       "caller": "carol", "caller_dept": "算法工程",
     "expected_owners": ["alice"], "expected_tags": [], "min_results": 1},
    ...
    {"id": "Q7",  "category": "跨人检索", "query": "全量发布",    "caller": "grace", "caller_dept": "算法工程",
     "expected_owners": ["alice"], "expected_tags": [], "min_results": 1},
    # 待办/决策检索
    {"id": "Q8",  "category": "决策检索", "query": "ai reco",  "caller": "carol", "caller_dept": "算法工程",
     "expected_owners": ["alice"], "expected_tags": ["ai-reco"], "min_results": 1},
    ...
    {"id": "Q11", "category": "人员检索", "query": "产品运营",    "caller": "alice", "caller_dept": "算法工程",
     "expected_owners": ["bob", "frank"], "expected_tags": ["产品运营"], "min_results": 1},
...
def _meeting_tags(meeting: dict) -> List[str]:
    ...
    topic = meeting["topic"]
    for kw in ["灰度", "扩量", "全量发布", "ModelDistill", "蒸馏", "决策树", "SDK", "性能优化",
                "QualityScore", "离线评估", "ai-reco", "ai reco"]:
...
GRAPH_QA = [
    ...
    {"query": "ai reco",   "caller": "david",  "caller_dept": "基础设施",
    ...
    {"query": "ModelDistill",        "caller": "grace",  "caller_dept": "算法工程",
